...
function action(mode, type, selection) {
   ...
   status--;
	...
	text += "\r\n#L1##r召唤怪物";
       ...
       cm.summonMob(9303015, 2600000, 70000, 1); //树妖王
        cm.dispose();
}
...
cm.dispose();
     ...
     cm.getC().getChannelServer().getWorldInterface().broadcastMessage(null, net.sf.odinms.tools.MaplePacketCreator.serverNotice(12,cm.getC().getChannel(),"[怪物总动员]" + " : " + " [" + cm.getPlayer().getName() + "]的小组完成了第18阶段挑战，进入了下一关卡！",true).getBytes()); 
     cm.dispose();
}
    }else if  (selection == 3) {      
           cm.sendOk("");
