...
function action(mode, type, selection) {
    if (mode == -1) {
        ...
        if (mode == 1)
            ...
            status--;
        if (status == 0) {
            cm.sendNext("嗨，我是 #b邮递员#k 我可以帮助你快速转职哦~~！");
        } else if (status == 1) {
		...
		cm.teachSkill(5001005,0,0);  //避免战神左左and右右出现疾驰效果
		cm.teachSkill(21120002,0,20); //战神之舞	
		cm.teachSkill(21120009,0,20); //(隐藏) 战神之舞- 双重重击
		cm.teachSkill(21120010,0,20); //(隐藏) 战神之舞 
                cm.teachSkill(21110002,1,20); //全力挥击
		cm.teachSkill(21121000,0,30); //爆击强化
		...
		cm.teachSkill(5001005,0,0);  //避免战神左左and右右出现疾驰效果
                ...
                cm.serverNotice("[转职系统]: 恭喜 [" + cm.getPlayer() + "] 战神四转成功!!!");
                cm.sendOk("转职成功！希望您以后的冒险之路顺利！");
            }
            cm.dispose();
        } else {
