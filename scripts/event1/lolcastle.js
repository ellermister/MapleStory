...
function mesoDistribution() {
	...
	while (iter.hasNext()) {
		...
		if (eim.getPlayerCount() > 0) {
			...
			map.broadcastMessage(net.sf.odinms.tools.MaplePacketCreator.serverNotice(6, "[公告事项] 恭喜 " + winner.getName() + " 玩家 获得了 " + meso + " 金币"));
...
function playerDead(eim, player) {
	...
	player.getClient().getSession().write(net.sf.odinms.tools.MaplePacketCreator.serverNotice(1, "你失去了 " + eim.getKillCount(player) + " 点。"));
}
...
function allMonstersDead(eim) {
	...
	while (iter.hasNext()) {
		...
		winner.getClient().getSession().write(net.sf.odinms.tools.MaplePacketCreator.serverNotice(1, "你获取了 " + eim.getKillCount(winner) + " 点券\r\n你将在2分钟内传送出。"));
