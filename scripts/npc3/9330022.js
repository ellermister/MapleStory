	   ...
	   cm.warp(910000000); 
		...
		//cm.gainExp(30000);
	  	//cm.summonMob(9300184, 1000, 500, 1);//大臣10W血
                cm.dispose();
                }else if (cm.getChar().getsg2() == 18){ 
                             cm.sendOk("已经召唤了！如果打完了，请选择进入下一关。");
                             cm.dispose();
        }else {
        cm.getChar().setsg2(18); //第5关
       cm.summonMob(6400005, 3000000, 70000, 1); //树妖王
        cm.dispose();
}

    }else if  (selection == 2) {
if (cm.getChar().getsg2() <= 17){ 
                             cm.sendOk("并非是你召唤的怪物！");
                             cm.dispose();     
...
cm.dispose();
     ...
     cm.warpParty(970031900);//进入下一个地图
     cm.givePartyExp(6000);
     cm.showEffect("quest/party/clear");
     cm.getC().getChannelServer().getWorldInterface().broadcastMessage(null, net.sf.odinms.tools.MaplePacketCreator.serverNotice(12,cm.getC().getChannel(),"[怪物总动员]" + " : " + " [" + cm.getPlayer().getName() + "]的小组完成了第17阶段挑战，进入了下一关卡！",true).getBytes()); 
     cm.dispose();
}
    }else if  (selection == 3) {      
           cm.sendOk("");
