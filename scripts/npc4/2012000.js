...
function action(mode, type, selection) {
    if(mode < 1) {
        ...
        if (status == 1) {
            select = selection;
            cm.sendYesNo("你确定要购买 " + mapName2[select] + " 需要 " + (select == 0 ? 15 : 10) + " 小时分钟, 它会花费你 #b"+cost[select]+" 枫币#k. 请问你是否确定要购买 #b#t"+ticket[select]+"##k?");
        } else if(status == 2) {
            if (cm.getMeso() < cost[select] || !cm.canHold(ticket[select]))
                cm.sendOk("你确定你有 #b"+cost[select]+" 枫币#k? 如果有的话,我劝您检查下身上其他栏位看是否有没有满了.");
            else {
