    ...
    if (mode == -1) {
        ...
        if (status == 0) {
           ...
           // gsjb += "#L10##r[金币]#k 兑换 #v4021010#   -   (#r1000W = 1#b)#l\r\n";
            ...
            gsjb += "#L12##b#v4001126#兑换[抵用券]  #b比例 - (#r1 = 200#b)#l\r\n";
            gsjb += "#L13##b#v4000313#兑换[点券]  #b比例 - (#r1 = 50#b)#l\r\n";
           ...
           cm.sendGetNumber("请输入数量:\r\n#b比例 - (#r1 = 1#b)\r\n  当前数量: #c4031196# 个#r \r\n", 1, 1, 100000 ); }
            ...
            if (beauty == 1) {
                ...
                if (cm.haveItem(4001126, selection)){
                    ...
                    cm.sendOk("兑换成功。 [#r"+selection+"#k]枫叶 兑换了:[#r"+(selection*200)+"#k] 抵用券");
					cm.worldMessage(6,"玩家：["+cm.getName()+"]努力搬砖，在柯南那里兑换了："+(selection*200)+" 抵用券");
					 cm.dispose();
                } else {
                    cm.sendOk("您的输入的数量错误，无法兑换。");
                    cm.dispose()
                ...
                if (cm.haveItem(4000313, selection)){
					...
					cm.gainNX(selection*50);	//加减点券
					//cm.gainDY(selection*2); //抵用券加减
					cm.getChar().UpdateCash();   //
                    cm.sendOk("兑换成功。 [#r"+selection+"#k]黄金枫叶 兑换了:[#r"+(selection*50)+"#k] 点券");
					cm.worldMessage(6,"玩家：["+cm.getName()+"]努力搬砖，在柯南那里兑换了："+(selection*50)+" 点券");
					 cm.dispose();
                } else {
                    cm.sendOk("您的输入的数量错误，无法兑换。");
                    cm.dispose()
