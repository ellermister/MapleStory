...
function action(mode, type, selection) {
    ...
    if (status == 0) {
    cm.sendSimple("你有没有听过海滩与所谓的 #b黄金海滩#k, 这个地方在维多利亚呢?? 我可以现在帮助你到那个地方只需要 #b1500 枫币#k, 或者如果你有一张 #bVIP 到黄金海滩的票#k 那么就可以免费去..\r\n\r\n#L0##b 我愿意付 1500 枫币.#l\r\n#L1# 我有一张 VIP 到黄金海滩的票.#l\r\n#L2# 什么是VIP到黄金海滩的票#k?#l");
    } else if (status == 1) {
	if (selection == 0) {
	    cm.sendYesNo("所以你想付 #b1500 枫币#k 然后去黄金海滩??");
	} else if (selection == 1) {
	    status = 2;
	...
	if (cm.getMeso() < 1500) {
	    cm.sendNext("你没有足够的枫币也没有VIP票 滚吧!");
	    cm.dispose();
	} else {
	    cm.gainMeso(-1500);
